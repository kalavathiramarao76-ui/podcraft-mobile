...
Target Platforms: ${_platformController.text.isNotEmpty ? _platformController.text : 'TikTok, Instagram Reels, YouTube Shorts'}
''';
    final response = await AIService.generateClips(details);
    setState(() { _result = response; _loading = false; });
    if (mounted) context.read<AppProvider>().addContent('clips', 'Clips suggestions', response);
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('Clip Finder', style: GoogleFonts.inter(fontWeight: FontWeight.w700)),
      ...
      body: SingleChildScrollView(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Container(
            ...
            child: Row(children: [
              Icon(Icons.content_cut_rounded, color: const Color(0xFF8b5cf6)),
              const SizedBox(width: 12),
              Expanded(child: Text('AI identifies the most viral-worthy moments from your episode.',
                style: GoogleFonts.inter(fontSize: 13, color: Colors.white70))),
            ]),
          ).animate().fadeIn().slideY(begin: 0.1),
          const SizedBox(height: 24),
          _label('Episode Content / Key Moments *'),
          const SizedBox(height: 8),
          TextField(controller: _contentController, style: GoogleFonts.inter(color: Colors.white), maxLines: 6,
            decoration: const InputDecoration(hintText: 'Paste transcript or describe memorable moments, quotes, stories...')),
          const SizedBox(height: 20),
          _label('Target Platforms'),
          const SizedBox(height: 8),
          TextField(controller: _platformController, style: GoogleFonts.inter(color: Colors.white),
            decoration: const InputDecoration(hintText: 'e.g., TikTok, Reels, Shorts (default: all)')),
          const SizedBox(height: 24),
          SizedBox(width: double.infinity, height: 56,
            child: ElevatedButton.icon(
              ...
              label: Text(_loading ? 'Finding Clips...' : 'Find Clip Moments'),
            ),
          ),
          if (_result.isNotEmpty) ...[
            const SizedBox(height: 24),
            Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              _label('Suggested Clips'),
